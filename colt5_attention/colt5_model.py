...
class CoLT5Decoder(nn.Module):
    ...
    def forward(self, input_ids, encoder_hidden_states, context_mask=None, decoder_mask=None, keep_routing_history=False):
        x = self.embed_tokens(input_ids)
        for layer in self.layers:
            x = layer(x, encoder_hidden_states, context_mask=context_mask, decoder_mask=decoder_mask, keep_routing_history=keep_routing_history)
        return x
